...
function renderProsCons(pros, cons) {
  ...
  return `<div class="pc-grid">
    <div class="pc-box pc-pro"><div class="pc-label">Vorteile</div><ul>${proItems}</ul></div>
    <div class="pc-box pc-con"><div class="pc-label">Nachteile</div><ul>${conItems}</ul></div>
  </div>`;
}
  ...
  return `<div class="objekt-card">
    ...
    <div class="objekt-body">
      ...
      <div class="meta-row">
        ...
        ${obj.etage ? `<span class="meta-chip">${obj.etage}</span>` : ''}
        ${obj.baujahr ? `<span class="meta-chip">BJ ${obj.baujahr}</span>` : ''}
        ${obj.zustand ? `<span class="meta-chip">${obj.zustand}</span>` : ''}
      </div>
...
function buildHtml(data) {
  const anzahl = (data.objekte||[]).length;
...
:root{--greige-light:#F2EDE6;--greige-mid:#E0D8CE;--greige-dark:#C8BCB0;--black:#1A1814;--dark:#2C2822;--mid:#6B6560;--light:#9A948E;--divider:#E8E3DC;--white:#FFFFFF;--pro-bg:#F6F8F4;--pro-border:#C8D8BE;--pro-text:#3D5C30;--con-bg:#FAF7F4;--con-border:#D8C8B8;--con-text:#6B4830;}
...
.cover-count{font-family:'Playfair Display',serif;font-size:11pt;font-weight:400;color:var(--dark);margin-bottom:3mm;}
...
.cover-footer-firm{font-family:'Playfair Display',serif;font-size:7.5pt;font-weight:400;color:var(--black);margin-bottom:1mm;text-align:center;}
.content-pages{padding:18mm 18mm 22mm 18mm;}
...
.page-header-title{font-size:7pt;color:var(--light);}
...
.link-box{font-size:7pt;color:#4A6FA5;max-width:80mm;text-align:right;word-break:break-all;line-height:1.4;text-decoration:underline;}
</style></head><body>
<div class="cover"><div class="cover-content">
  <div class="cover-top"><div class="cover-brand">Suchprofil</div><div class="cover-datum">${data.datum}</div></div>
  <div class="cover-center">
    ...
    <div class="cover-meta-grid">
      <div class="cover-meta-item"><span class="cover-meta-label">Region</span><span class="cover-meta-value">${data.regionen}</span></div>
      <div class="cover-meta-item"><span class="cover-meta-label">Größe</span><span class="cover-meta-value">${data.groesse||'k.A.'}</span></div>
      <div class="cover-meta-item"><span class="cover-meta-label">Zimmer</span><span class="cover-meta-value">${data.zimmer||'k.A.'}</span></div>
      ${data.kundenEigenschaften ? `<div class="cover-meta-item"><span class="cover-meta-label">Besondere Anforderungen</span><span class="cover-meta-value">${data.kundenEigenschaften}</span></div>` : ''}
    </div>
  </div>
  <div class="cover-bottom"><div class="cover-footer-note">
...
export default function handler(req, res) {
  ...
  try {
    const data = { ...req.body, datum: datumDe() };
    const html = buildHtml(data);
    const filename = `Suchprofil_${(data.kunde||'Export').replace(/\s+/g,'_')}.html`;
