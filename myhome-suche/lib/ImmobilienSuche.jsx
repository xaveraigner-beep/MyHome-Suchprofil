...
${selectedDomains.slice(0, 4).map(domain =>
  `- "${objektStr} ${vertragsStr} ${region1} site:${domain}"`
).join("
")}
...
Extrahiere die besten 4 passenden Objekte aus allen Suchergebnissen.`;
    ...
    try {
      ...
      // Alle text-Blöcke aus der Response extrahieren (nach tool_use kommt das Ergebnis)
  ...
  const handlePdfExport = async () => {
    ...
    const prompt = `Du bist ein Python-Entwickler. Generiere ein professionelles PDF für einen Immobilien-Suchauftrag.

Verwende folgenden Python-Code mit WeasyPrint und gib NUR den vollständigen ausführbaren Python-Code zurück, kein Markdown.
...
${JSON.stringify({
  ...
  regionen: alleRegionen,
  vertragsart: form.vertragsart,
  groesse: form.groesseVon || form.groesseBis ? \`\${form.groesseVon||""}–\${form.groesseBis||""} m²\` : "k.A.",
  zimmer: form.zimmerMin ? \`\${form.zimmerMin}+\` : "k.A.",
  objekte: exportObjekte,
}, null, 2)}

Nutze exakt dieses Design-Template und fülle die Daten ein. Das PDF soll gespeichert werden als /mnt/user-data/outputs/suchprofil_export.pdf`;
    ...
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514",
          max_tokens: 4000,
          system: `Du generierst Python WeasyPrint PDF-Code. Antworte NUR mit ausführbarem Python-Code, kein Markdown, keine Erklärung, keine Backticks.

Verwende dieses exakte HTML/CSS Template für das PDF:
- Greige Deckblatt (#F2EDE6) mit vier Ecken ohne Dreiecke
- Playfair Display für Überschriften, Inter für Fließtext
- Haupttitel: "Suchprofil" (36pt), Unterüberschrift: "X ausgewählte Objekte" (11pt), darunter kursiv Objekttyp in Region (11pt italic), darunter Kundenname
- Firmenblock zentriert unten: "My Home is my Home" / Immobilien Wolfgang Schiessendoppler & Partner KG / Fürbergstraße 42A · 5020 Salzburg · FN 407346 p
- Inhaltsseiten: weißer Hintergrund, schwarzer Header pro Karte mit Nummer + Preis, Kurzbeschreibung kursiv, Meta-Chips in Greige, Pro/Con nebeneinander grün/braun
- Links in #4A6FA5 klickbar
- Datum auf Deutsch

Importiere: weasyprint, datetime. Output-Pfad: /mnt/user-data/outputs/suchprofil_export.pdf`,
          messages: [{ role: "user", content: prompt }],
        }),
      });

      const data = await response.json();
      const pyCode = (data.content || []).filter(b => b.type === "text").map(b => b.text).join("").trim();

      // Send code to Claude for execution via the computer tool
      // For now: download as Python file that user can run
      const blob = new Blob([pyCode], { type: "text/plain" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "generate_suchprofil.py";
      a.click();
